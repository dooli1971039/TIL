...
class BMIActivity : AppCompatActivity() {
	...
	private fun calculateUnits(){
		if (currentVisibleView == METRIC_UNITS_VIEW) {
			if (validateMetricUnits()) {  //입력을 제대로 했으면
				val heightValue: Float = binding?.etMetricUnitHeight?.text.toString().toFloat() / 100  //미터 단위로 구하려고
	...
	private fun validateMetricUnits(): Boolean { // 입력했는지 아닌지 체크
		var isValid = true
	...
	private fun displayBMIResult(bmi: Float) {
		...
		//꼭 compareTo 안 써도 됨
		if (bmi.compareTo(15f) <= 0) { //bmi가 15보다 작거나 같으면
		...
		binding?.llDiplayBMIResult?.visibility = View.VISIBLE // Visible 하게 바꿈

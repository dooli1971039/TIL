...
// 아이템 리스트를 매개변수로 받아온다. 어댑터 클래스를 상속 받는다.(뷰홀더도 넣어줄 것)
...
//		fun onRemoveAlbum(position: Int)
	...
	fun addItem(album: Album){
		albumList.add(album)
		notifyDataSetChanged() //추가하면 RV한테 꼭 데이터가 바꼈다고 알려줘야 함
	}

	fun removeItem(position: Int){
		albumList.removeAt(position)
		notifyDataSetChanged() //삭제하면 RV한테 꼭 데이터가 바꼈다고 알려줘야 함
	}

	// 뷰홀더에 데이터를 바인딩해줘야 할 때마다 호출되는 함수 => 엄청나게 많이 호출
	// 클릭 이벤트도 여기서 주로 처리 - 클릭리스너에 대한 인터페이스를 구현해야 함
   ...
   //     holder.binding.itemAlbumTitleTv.setOnClickListener { mItemClickListener.onRemoveAlbum(position) } //삭제됐을 때
	...
	// 뷰홀더 - 아이템뷰 객체를 재활용 하기 위해 담고있는 변수들
